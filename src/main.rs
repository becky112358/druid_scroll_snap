...
use druid::{
    theme, AppLauncher, Color, Env, FontDescriptor, FontWeight, Widget, WidgetExt, WindowDesc,
};

mod list_snap;
use crate::list_snap::{ListSnap, ScrollSnap};
mod types;
use types::AppData;
...
fn build_list() -> impl Widget<AppData> {
    Flex::column().with_flex_child(
        Scroll::new(ListSnap::new(List::new(build_individual_item)).lens(AppData::indices))
            .vertical()
            .controller(ScrollSnap {}),
        1.0,
    )
}
